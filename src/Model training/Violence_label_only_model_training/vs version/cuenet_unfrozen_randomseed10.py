...
save_path      = r"D:\UK\00. 2024 QMUL\00. Course\SAV-ViolenceDetection\Results\Model Training(w.o video)\CUE-NET"
...
class CUENet(nn.Module):
    ...
    def forward(self, x):
        B, T, C, H, W = x.shape
        x = x.view(B*T, C, H, W)
        feats = checkpoint(self.backbone, x, use_reentrant=False)  # memory efficient
